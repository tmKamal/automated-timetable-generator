  ...
  const submitHandler = async (e) => {
    ...
    try {
      const responseData = await sendRequest(
        `http://localhost:8000/api/session/paralSessions`,
        "POST",
        ...
        <Paper className={classes.paper}>
          ...
          <form onSubmit={submitHandler} className={classes.form} noValidate>
            ...
            <Grid item xs={12}>
                ...
                >
                  <InputLabel id="groupType">Session Type</InputLabel>
                  <Select
                    labelId="groupType"
                    id="groupType"
                    value={sessionType}
                    onChange={onChangeHandler("sessionType")}
                    label="groupType"
                  >
              ...
              {sessionType =="Lecture" &&<Grid item xs={12}>
                <FormControl
                  style={{ width: "552px" }}
                  variant="outlined"
                  className={classes.formControl}
                >
                  ...
                  <Select
                    ...
                    multiple
                    value={sessionArray}
                    onChange={onChangeHandler("sessionArray")}
                    input={<Input />}
                  ...
                  >
                    {!isLoading &&
                      sessionData &&
                      sessionData.map((s) =>
                        s.tag.tagType == "Lecture" ? (
                          <MenuItem key={s.id} value={s.id}>
                            <Checkbox
                              checked={sessionArray.indexOf(s.id) > -1}
                            />
                            <ListItemText primary={s.subjectCode+' '+s.tag.tagType+' - '+s.studentGroup.academicYearSem+'.'+s.studentGroup.groupNumber} />
                          </MenuItem>
                        ) : null
                      )}
                  </Select>
              ...
              {sessionType =="Tutorial" &&<Grid item xs={12}>
                <FormControl
                  style={{ width: "552px" }}
                  variant="outlined"
                  className={classes.formControl}
              ...
              {sessionType =="Practical" &&<Grid item xs={12}>
                <FormControl
                  style={{ width: "552px" }}
                  variant="outlined"
                  className={classes.formControl}
                        ...
                        s.tag.tagType == "Practical" ? (
                          <MenuItem key={s.id} value={s.id}>
                            <Checkbox
                              checked={sessionArray.indexOf(s.id) > -1}
                            />
                            <ListItemText primary={s.subjectCode+' '+s.tag.tagType+' - '+s.subGroup.academicYearSem+'.'+s.subGroup.groupNumber+'.'+s.subGroup.subGroupNumber} />
                          </MenuItem>
                        ) : null
                      )}
                  </Select>
