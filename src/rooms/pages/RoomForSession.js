  ...
  const submitHandler = async (e) => {
    ...
    try {
      const responseData = await sendRequest(
        `http://localhost:8000/api/session/room/${sessionId}`,
        "PATCH",
              ...
              <Grid item xs={12}>
                ...
                >
                  ...
                  >
                    {!isLoading &&
                      sessionData &&
                      sessionData.map((s) => s.tag.tagType=='Practical'?
                        
                          <MenuItem key={s.id} value={s.id}>
                            {s.subjectCode+' '+s.tag.tagType+' - '+s.subGroup.academicYearSem+'.'+s.subGroup.groupNumber+'.'+s.subGroup.subGroupNumber}
                          </MenuItem>
                        : <MenuItem key={s.id} value={s.id}>
