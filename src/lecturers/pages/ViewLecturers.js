...
const ViewLecturers = () => {
  ...
  useEffect(() => {
    const fetchingLecturers = async () => {
      try {
        setLoadedLecturers(
          await sendRequest("http://localhost:8000/api/lecturer")
        );
      } catch (err) {}
    };
    fetchingLecturers();
    ...
    const deleteLecturer = async () => {
        try {
          const delMsg = await sendRequest(
            `http://localhost:8000/api/lecturer/${deleteId}`,
            "DELETE",
            null,
            {  }
          );
              ...
              {!isLoading &&
                ...
                loadedLecturers.lecturers.map((lecturer) => (
                  <TableRow key={lecturer.id}>
                    ...
                    <TableCell align="center">{lecturer.empId}</TableCell>
                    <TableCell align="center">{lecturer.faculty}</TableCell>
                    <TableCell align="center">{lecturer.department}</TableCell>
                    <TableCell align="center">{lecturer.center}</TableCell>
                    <TableCell align="center">{lecturer.building.buildingName}</TableCell>
                    <TableCell align="center">{lecturer.level}</TableCell>
                    <TableCell align="center">{lecturer.rank}</TableCell>
                    <TableCell align="center">
                      <Link to={`/update-lecturer/${lecturer.id}`}>
                        <IconButton
                    ...
                    <TableCell align="center">
                      <IconButton
                        onClick={() => deleteDialogOpener(lecturer.id)}
                        color="secondary"   
